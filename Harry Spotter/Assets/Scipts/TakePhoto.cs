...
public class TakePhoto : MonoBehaviour
{
    ...
    private Text sendinObjectTeaxtTemp;
    [SerializeField] private GameObject _findAnotherItemButton;
    ...
    public void GetObjectIds(List<string> ids)
    {
        ...
        //backend error no object id check 
        if(ids[0] == "NoObjectId")
        {
            if (_sendingObjectText.activeSelf)
            {
                sendinObjectTeaxtTemp = _sendingObjectText.GetComponent<Text>();
                sendinObjectTeaxtTemp.color = Color.red;
                sendinObjectTeaxtTemp.text = "This object was not accepted, please try a different object.";
                _nextButton.SetActive(false);
                _findAnotherItemButton.SetActive(true);
            }
        } else
        {
            if (_sendingObjectText.activeSelf)
